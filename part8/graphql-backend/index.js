...
const resolvers = {
  Query: {
    ...
    allBooks: (_, { author, genre }) =>
      books
        .filter((b) => (author ? b.author === author : true))
  ...
  Mutation: {
    addBook: (_, { title, author, published, genres }) => {
      if (!authors.find((a) => a.name === author)) {
        authors.push({ id: uuid(), name: author, born: null })
      }
      const book = { id: uuid(), author, title, published, genres }
      books.push(book)
